...
WT40 <- WT40[,sort(colnames(WT40))]

WT40.meta <- WT40.meta %>% arrange(Names)
...
write.csv(resOrderedDF_top100, file = "results/WT40/results_top100.csv")

# only significant results
write.csv(resSig[order(resSig$pvalue),], file = "results/WT40/sig_results.csv")

# resSchurch results
write.csv(resSigSchurch[order(resSigSchurch$pvalue),], file = "results/WT40/Schurch_results.csv")

# save all genes
write.csv(res.vgp[order(res.vgp$pvalue),], file = "results/WT40/all_results.csv")

# makinghtml report
library("ReportingTools")
htmlRep <- HTMLReport(shortName="report", title="My report",
                      reportDirectory="results/WT40/report")
publish(res.vgp, htmlRep)
url <- finish(htmlRep)
browseURL(url)
