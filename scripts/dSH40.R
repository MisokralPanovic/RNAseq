...
dSH40 <- dSH40[,sort(colnames(dSH40))]

dSH40.meta <- dSH40.meta %>% arrange(Names)
...
nrow(dSH40)
dSH40 <- dSH40[rownames(dSH40) %like% "ENS", ]
nrow(dSH40)

# check if count sample names and metdata sample names are the same
all(rownames(dSH40.meta) %in% colnames(dSH40))

write.csv(dSH40, file = "Data/dSH40.csv", row.names = TRUE)

# DESeq2 object creation ----------------------
dds <- DESeqDataSetFromMatrix(countData = dSH40,
                              colData = dSH40.meta,
                              design = ~ Condition)
...
pheatmap(normalised_counts,
         color =  pal,
         cluster_rows = T,
         show_rownames = F,
         annotation = select(dSH40.meta, Condition),
         scale = "row")

## LFC shrinkage (if low counts)
res <- results(dds)
res.shrink <- DESeq2::lfcShrink(dds,coef = 2, res = res) 
summary(res.shrink)
                                
                        
# contrasts = c("Condition", "untreated", "treated") #contrast: what metadata column, changed, base
...
write.csv(resOrderedDF_top100, file = "results/dSH40/results_top100.csv")

# only significant results
write.csv(resSig[order(resSig$pvalue),], file = "results/dSH40/sig_results.csv")

# resSchurch results
write.csv(resSigSchurch[order(resSigSchurch$pvalue),], file = "results/dSH40/Schurch_results.csv")

# save all genes
write.csv(res.vgp[order(res.vgp$pvalue),], file = "results/dSH40/all_results.csv")

# makinghtml report
library("ReportingTools")
htmlRep <- HTMLReport(shortName="report", title="My report",
                      reportDirectory="results/dSH40/report")
